...
from master_project.networks.layers.encoder import (
    QuaternionEncoder,
    ScalarEncoder,
    PositionEncoder,
)
from master_project.observation import MasterObservation
from master_project.registry.state import State, StateType
...
class GnnBase(ActorCriticBase, ABC):
    ...
    def task_state_distances(
        ...
        for task in self.tasks:
            distances = task.state_distances(obs, goal, self.states, pad, sparse)
