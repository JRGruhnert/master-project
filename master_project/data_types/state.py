...
class State(ABC):
    ...
    def _create_state_by_type(
        cls,
        name: str,
        state_type: StateDistance,
        success_mode: StateSuccess,
    ) -> "State":
        """Factory method using registry"""
        if 
        type not in cls._state_registry:
    ...
    def from_json(cls, name: str, json_data: dict) -> "State":
        ...
        state_type = StateDistance(json_data["type"])
    ...
    def __init__(
        ...
        self._success_mode = success_mode
    ...
    @property
    def state_type(self) -> StateDistance:
...
class State(ABC):
    ...
    def __init__(
        ...
        if self._type == StateDistance.Euler_Angle or self._type == StateDistance.Axis_Angle:
            assert self._lower_bound.shape == self._upper_bound.shape == (3,)
        elif self._type == StateDistance.Range:
            assert self._lower_bound.shape == self._upper_bound.shape == (1,)
        if self._type != StateDistance.Euler_Angle and self._type != StateDistance.Axis_Angle:
            assert (
...
@State.register_type(StateDistance.Euler_Angle)
...
@State.register_type(StateDistance.Quaternion)
...
@State.register_type(StateDistance.Range)
...
@State.register_type(StateDistance.Boolean)
...
@State.register_type(StateDistance.Flip)
