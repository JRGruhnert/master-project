...
def plot(collection: RunDataCollection):
    ...
    for nt in [NT_GNN, NT_MLP]:
        for domain in data["domains"]:
            ...
            data[nt + "_train"].append(run_t.stats["run_stats"]["max_sr"])
            ...
            if domain != "sr":
                ...
                data[nt + "_retrain_eval"].append(run_re.stats["run_stats"]["max_sr"])
                data[nt + "_retrain_domain"].append(run_rd.stats["run_stats"]["max_sr"])
            else:
                data[nt + "_retrain"].append(0.0)
                data[nt + "_retrain_eval"].append(0.0)
                data[nt + "_retrain_domain"].append(0.0)
